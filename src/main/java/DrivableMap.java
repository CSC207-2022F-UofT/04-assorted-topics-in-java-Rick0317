...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed){
        int i;
        for(i = 0; i< this.drivable_map.values().size();i++){
            if(((Drivable) this.drivable_map.values().toArray()[i]).getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable(){
        ...
        for(i = 0; i<this.drivable_map.values().size();i++){
            if(this.drivable_map.values().toArray()[i] instanceof Tradable){
                tradables.add((Tradable) this.drivable_map.values().toArray()[i]);
            }
        }
        return tradables;
    }
